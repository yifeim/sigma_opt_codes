function equation = powerlawplot(Net, opts)
% Power-Law Degree Distribution Graphing

if nargin<2, opts=struct(); end
titlename = get_option(opts, 'title', inputname(1));

if graphconncomp(sparse(logical(Net)))>1, warning('not connected graph'); end

% Finds out how many connections each node has
degrees = round(full(sum(Net)));

x = setdiff(unique(degrees),0);
y = histc(degrees, x);
...
%     P(1,T) = T;
%     if degrees(1,T) ~= 0
%         frequency(1,degrees(1,T)) = frequency(1,degrees(1,T)) + 1;
%     end
% end
% 
