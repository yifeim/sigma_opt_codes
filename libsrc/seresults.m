function [listofHeurs, varargout] = seresults( ...
...
for qitr=1:length(listofQualities)
  for hitr = 1:length(listofHeurs)
    meanthisQ(hitr, :) = arrayfun( ...
      @(c) thesefun(rgrsn_evals, listofHeurs{hitr}, listofQualities{qitr}, c), ...
      1:queryLen);
  end
...
function [number] = thesefun(rgrsn_evals, hname, qname, c)
number = std(cellfun(@(r) r.(hname)(c).(qname), rgrsn_evals)) ...
  / sqrt(length(rgrsn_evals));
end
